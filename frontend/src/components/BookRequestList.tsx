...
import { getUserBookBorrowing } from "../services/userService";
    ...
    const fetchRequests = async () => {
      try {
        ...
        if (
          ...
          data = await getBorrowing(mapStatusToString(statusFilter), 1, 10);
        } else if (
          (decodedToken as { [roleString]: string })[roleString] === "User"
        ) {
          data = await getUserBookBorrowing(
            (decodedToken as { id: string }).id,
            mapStatusToString(statusFilter),
            1,
            10
          );
        }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 border-b border-gray-800">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Manage and view all book requests
          </p>
        </div>
            ...
            {expandedRequestId === request.id && (
              <div className="mt-4 space-y-4">
                ...
                {request.status === BookRequestStatus.Waiting &&
                  (decodedToken as { [roleString]: string })[roleString] ===
                    "SuperUser" && (
